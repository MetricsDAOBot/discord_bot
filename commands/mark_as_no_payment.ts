...
module.exports = {
	data: new SlashCommandBuilder()
		.setName('mark_as_no_payment')
		.setDescription('Marks the approved request as no additional payment needed! (Only usable in request posts)'),
	async execute(interaction: ChatInputCommandInteraction<CacheType>, client: CustomClient) {
		try {
			let { user } = interaction;
            await interaction.deferReply({ ephemeral: true });

            let res = await axios.post<RegradeRequest | string>('/mark_request_as_no_payment', {
                discord_id: user.id,
			...
			console.log(e);
            await deleteReplyInteractionAfterSeconds(interaction, "Unable to mark as no payment.", 5, "update");
		}
	},
};